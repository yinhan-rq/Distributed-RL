...
from .misc import discrete_autoregreesive_act
from .misc import discrete_parallel_act
...
class Encoder(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, n_block, n_embd, n_head, n_agent, encode_state):
        ...
        self.head = nn.Sequential(init_(nn.Linear(n_embd, n_embd), activate=True), nn.GELU(), nn.LayerNorm(n_embd), init_(nn.Linear(n_embd, 1)))
...
class Decoder(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, n_block, n_embd, n_head, n_agent, action_type='Discrete', dec_actor=False, share_actor=False):
        ...
        self.head = nn.Sequential(init_(nn.Linear(n_embd, n_embd), activate=True), nn.GELU(), nn.LayerNorm(n_embd), init_(nn.Linear(n_embd, action_dim)))
...
class MultiAgentTransformer(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent, action_type='Discrete'):
        super(MultiAgentTransformer, self).__init__()

        device = torch.device(g_args("device"))
        self.n_agent = g_args("left_agent_num")
        self.action_dim = action_dim
        ...
        # state unused
        state_dim = 37
        n_block = g_args("n_block")
        n_embd = g_args("n_embd")
        n_head = g_args("n_head")
        encode_state = g_args("encode_state")
        dec_actor = g_args("dec_actor")
        share_actor = g_args("share_actor")

        self.encoder = Encoder(state_dim, obs_dim, n_block, n_embd, n_head, n_agent, encode_state)
        self.decoder = Decoder(obs_dim, action_dim, n_block, n_embd, n_head, n_agent, self.action_type, dec_actor, share_actor)
        self.to(device)
