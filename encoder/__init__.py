...
class FeatureEncoder():
    ...
    def Encoder(self, obs):
        player_id = obs['active'] # {0..N-1} integer denoting index of the controlled players.
        player_pos_x, player_pos_y = obs['left_team'][player_id]
        player_direction = np.array(obs['left_team_direction'][player_id])
        player_speed = np.linalg.norm(player_direction)
        player_role = obs['left_team_roles'][player_id]
        player_role_onehot = self._encode_role_onehot(player_role)
        player_tired = obs['left_team_tired_factor'][player_id]
        ...
        ball_distance = np.linalg.norm([ball_x_relative, ball_y_relative])   # 球距离控制者的位置
        ball_speed = np.linalg.norm([ball_x_speed, ball_y_speed])  # 球速
        ball_owned = 0.0 
        if obs['ball_owned_team'] == -1:  # ball no owned
            ball_owned = 0.0
        else:
            ball_owned = 1.0
        ball_owned_by_us = 0.0
        if obs['ball_owned_team'] == 0:  # ball own left team
            ball_owned_by_us = 1.0
        elif obs['ball_owned_team'] == 1: # right team
            ball_owned_by_us = 0.0
        else:
            ball_owned_by_us = 0.0

        ball_which_zone = self._encode_ball_which_zone(ball_x, ball_y)  # 将球场划分为5个区域，这个暂时可以不用
        ...
        player_state = np.concatenate((avail[2:], obs['left_team'][player_id], player_direction*100, [player_speed*100],
        ...
        obs_left_team = np.delete(obs['left_team'], player_id, axis=0)   # 去除该球员
        obs_left_team_direction = np.delete(obs['left_team_direction'], player_id, axis=0)
        left_team_relative = obs_left_team
        left_team_distance = np.linalg.norm(left_team_relative - obs['left_team'][player_id], axis=1, keepdims=True)
        left_team_speed = np.linalg.norm(obs_left_team_direction, axis=1, keepdims=True)
        left_team_tired = np.delete(obs['left_team_tired_factor'], player_id, axis=0).reshape(-1,1)
        ...
        left_closest_idx = np.argmin(left_team_distance)  # 距离控制球员最近的队友
        ...
        right_team_distance = np.linalg.norm(obs_right_team - obs['left_team'][player_id], axis=1, keepdims=True) # 对手离控制球员的距离
    ...
    def _get_avail(self, obs, ball_distance):
        ...
        if obs['ball_owned_team'] == 1: # opponents owning ball
            avail[LONG_PASS], avail[HIGH_PASS], avail[SHORT_PASS], avail[SHOT], avail[DRIBBLE] = 0, 0, 0, 0, 0  # 球在对手手上，就无法传球和射门、运球
